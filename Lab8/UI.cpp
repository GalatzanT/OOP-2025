...
void printMenu() {
	cout << "Lab 8 Farmacie Tudor Galatan\n";
	...
	cout << "8.Genereaza reteta\n";
	cout << "9.Adauga medicament in reteta\n";
	cout << "10.Goleste reteta\n";
	cout << "11.Salveaza reteta in fisier\n";
	cout << "12.Afiseaza reteta\n";
	cout << "13.Iesire\n";
	cout << "14.Raport reteta\n";
}
...
void afiseaza_medicamente2(const vector<const Medicament*>& medicamente) {
	for (const auto* medicament : medicamente) {
		cout << *medicament << endl;
	}
}
...
void afiseaza_nr_elemente_reteta(Service& s) {
	cout <<"Numarul de medicamente din reteta este: " << s.get_reteta_nr_med() << "\n";
}


void ui_genereaza_reteta(Service& s) {
	cout << "Nr de medicamente reteta:";
	int nrMed;
	cin >> nrMed;
	cin.ignore();
	s.genereaza_reteta(nrMed);
	afiseaza_nr_elemente_reteta(s);

}

void ui_goleste_reteta(Service& s) {
	s.goleste_reteta();
	afiseaza_nr_elemente_reteta(s);
}
void ui_adauga_med_reteta(Service& s) {
	cout << "Denumirea medicamentului:";
	string den;
	getline(cin, den);
	if (s.adauga_med_reteta(den) == 1) {
		cout << "Medicament adaugat in reteta cu succes\n";
	}
	else {
		cout << "Nu am gasit medicamentul cu denumirea data\n";
	}
	afiseaza_nr_elemente_reteta(s);
}

void salveaza_reteta_fisier(Service& s){
	cout << "Numele fisierului:";
	string numeFisier;
	getline(cin, numeFisier);
	try {
		s.salveaza_reteta_fisier(numeFisier);
		cout << "Retata s-a salvat in fisierul: " << numeFisier << " \n";
	}
...
void afiseaza_raport(const vector<const Medicament*> reteta) {
	RetetaDTO raport;
	raport.fa_raport(reteta);

	for (const auto& entry : raport.get_map()) {
		std::cout << entry.first << " apare de " << entry.second << " ori\n";
	}
}
...
void static run() {
	...
	while (true) {
		...
		if (opt > 0 && opt < 15)
			...
			case 8: {
				ui_genereaza_reteta(s);
				break;
			}
			case 9: {
				ui_adauga_med_reteta(s);
				break;
			}
			case 10: {
				ui_goleste_reteta(s);
			...
			case 11: {
				salveaza_reteta_fisier(s);
				break;
			}

			case 12: {
				afiseaza_medicamente2(s.get_reteta_all());
				break;
			}
			case 13: {
				cout << "Iesire...\n";
			...
			case 14: {
				afiseaza_raport(s.get_reteta_all());
				break;
			}
			}
		else {
			cout << "Un intreg intre 1-14!\n";
			continue;
		}
	}
}
