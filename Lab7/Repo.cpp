...
void Repo::update(int id, const Medicament& newMedicament) {
    for (auto& medicament : this->medicamente) { // Ensure medicament is not const
        if (medicament.getId() == id) {
            medicament = newMedicament; // Assignment works now
            return;
        }
    }
    throw RepoException("Medicament not found");
...
MyVector<Medicament>& Repo::getAll() {
...
void testRepo() {
    ...
    // 🔹 Testăm `getAll`
    MyVector<Medicament> all = repo.getAll();
